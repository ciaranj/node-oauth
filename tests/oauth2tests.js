...
vows.describe('OAuth2').addBatch({
  'Given an OAuth2 instance with clientId and clientSecret, ': {
    ...
    'When adding event listener to request:before event': {
      'we should raise an event before a request is sent and wait for method callback': function (oa) {
        oa._executeRequest = function (http_library, options, post_body, callback) {
          oa._executeRequest = new OAuth2('clientId', 'clientSecret')._executeRequest.bind(oa); // reset to original state
          assert.equal(options.headers['test-header'], 'test');
          assert.equal(options.headers['kidding-header'], ':p');
        };
        // oa._executeRequest(httpLibrary, {}, null, function (err, result, response) {})
        // we register a new event to handle informations from last time before execute request
        oa.on('request:before', function (options, post_body, doneFunction) {
          options.headers['test-header'] = 'test';
          doneFunction(options, post_body);
        });

        oa.on('request:before', function (options, post_body, doneFunction) {
          options.headers['kidding-header'] = ':p';
          doneFunction(options, post_body, true);
        });

        oa._request('POST', 'https://www.someurl.com/auth', {}, null, '', function (error, data, response) {
          assert.ok((error === null));
          oa.removeAllListeners('request:before');
        });
      }
    },
    'When adding event listener to request:after event': {
      'we should raise an event after a request is sent': function (oa) {
        oa._chooseHttpLibrary = function (parseUrl) {
          return {
            request: function () {
        ...
        oa.on('request:after', function (res, data) {
        ...
        oa._request('POST', 'https://www.someurl.com/auth', {}, null, '', function (error, data, response) {
          oa._chooseHttpLibrary = new OAuth2('clientId', 'clientSecret')._chooseHttpLibrary.bind(oa); // reset to original state
        });
      }
    },
    'When dealing with the response from the OP': {
      'we should treat a 201 response as a success': function (oa) {
        oa.removeAllListeners('request:after');
    ...
    'When we use the authorization header': {
      'and call get with the default authorization method': {
        'we should pass the authorization header with Bearer method and value of the accessToken, _request should be passed a null accessToken': function (oa) {
          oa._request = function (method, url, headers, postBody, accessToken, callback) {
      ...
      'and call get with the authorization method set to Basic': {
        'we should pass the authorization header with Basic method and value of the accessToken, _request should be passed a null accessToken': function (oa) {
          oa._request = function (method, url, headers, postBody, accessToken, callback) {
    ...
    'When we do not use the authorization header': {
      'and call get': {
        'we should pass NOT provide an authorization header and the accessToken should be being passed to _request': function (oa) {
          oa._request = function (method, url, headers, postBody, accessToken, callback) {
